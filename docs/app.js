...
function stripCexImprovements(text) {
  ...
  const start5 = normalizedText.indexOf(`(lp0${lineEnding}(iMedidasDeMejora.objetoGrupoMejoras`);
  if (start5 < 0) return text;

  const start6 = normalizedText.indexOf(`(lp0${lineEnding}(lp1${lineEnding}a(lp2${lineEnding}V`, start5 + 1);
  const emptyMarker = `(lp0${lineEnding}.`;
  const start7 = normalizedText.indexOf(emptyMarker, start6 + 1);
  const start8 = normalizedText.indexOf(emptyMarker, start7 + 1);
  const start9 = normalizedText.indexOf(emptyMarker, start8 + 1);
  const start10 = normalizedText.indexOf(`(lp0${lineEnding}VSin `, start9 + 1);
  const start11 = normalizedText.indexOf(`(lp0${lineEnding}VCONJUNTO DE MEJORAS 1`, start10 + 1);
  const start12 = normalizedText.indexOf('(dp0', start11 + 1);
  if ([start6, start7, start8, start9, start10, start11, start12].some(index => index < 0)) return text;

  // Keep CE3X's legacy protocol-0 shape while removing inherited packages.
  const emptyList = '(l.';
  const blankValue = cexPickleString('').replace(/\r?\n/g, lineEnding);
  const blankValues = `${blankValue}${Array.from({ length: 9 }, () => `a${blankValue}`).join('')}`;
  const emptySummary = `(l(la(l${blankValues}aa(laNa.`;
  const emptyMetadata = `${serializeCexList(Array.from({ length: 5 }, () => blankValue)).replace(/\r?\n/g, lineEnding)}(la(la.`;
  return [
    normalizedText.slice(0, start5),
    emptyList,
    emptySummary,
    emptyList,
    emptyList,
    emptyList,
    normalizedText.slice(start10, start11),
    emptyMetadata,
    normalizedText.slice(start12),
  ].join('');
}

function splitCexTopLevelStreams(text, lineEnding) {
  const marker = value => `(${value}`;
  const find = (value, from) => text.indexOf(value, from);
  const starts = [];
  const add = (value, from) => {
    const start = find(value, from);
    if (start < 0) return false;
    starts.push(start);
    return true;
  };

  if (!add("S'CEXv2.3 Residencial'", 0)) return [];
  if (!add(`${marker(`lp0${lineEnding}V`)}`, starts.at(-1) + 1)) return [];
  if (!add(`${marker(`lp0${lineEnding}V`)}`, starts.at(-1) + 1)) return [];
  if (!add(`${marker(`lp0${lineEnding}(lp1${lineEnding}(lp2${lineEnding}VCubierta`)}`, starts.at(-1) + 1)) return [];
  if (!add(`${marker(`lp0${lineEnding}(lp1${lineEnding}(lp2${lineEnding}V`)}`, starts.at(-1) + 1)) return [];
  if (!add(`${marker(`lp0${lineEnding}(iMedidasDeMejora.objetoGrupoMejoras`)}`, starts.at(-1) + 1)) return [];
  if (!add(`${marker(`lp0${lineEnding}(lp1${lineEnding}a(lp2${lineEnding}V`)}`, starts.at(-1) + 1)) return [];

  const emptyMarker = marker(`lp0${lineEnding}.`);
  for (let index = 0; index < 3; index += 1) {
    if (!add(emptyMarker, starts.at(-1) + 1)) return [];
  }
  if (!add(`${marker(`lp0${lineEnding}VSin `)}`, starts.at(-1) + 1)) return [];
  if (!add(`${marker(`lp0${lineEnding}VCONJUNTO DE MEJORAS 1`)}`, starts.at(-1) + 1)) return [];
  if (!add(`${marker(`dp0`)}`, starts.at(-1) + 1)) return [];
  if (!add(`I01${lineEnding}.S'`, starts.at(-1) + 1)) return [];
  if (!add(`S'`, starts.at(-1) + 1)) return [];
...
function applyCexAdminReplacements(text, record) {
  ...
  const normalizedText = String(text).replace(/\r?\n/g, lineEnding);
  const headerStart = normalizedText.indexOf("S'CEXv2.3 Residencial'");
  const adminStart = normalizedText.indexOf(`(lp0${lineEnding}V`, headerStart + 1);
  const generalStart = normalizedText.indexOf(`(lp0${lineEnding}V`, adminStart + 1);
  if (adminStart >= 0 && generalStart > adminStart) {
    const adminStream = replacements.reduce(
      (current, [memo, replacement]) => replaceFirstMemoValue(current, memo, replacement),
      normalizedText.slice(adminStart, generalStart),
    );
    return normalizedText.slice(0, adminStart) + adminStream + normalizedText.slice(generalStart);
  }
